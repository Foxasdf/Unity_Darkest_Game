...
class CodeSnippets : AssetPostprocessor
{
	...
	public static void Substitute(ref string part, SymbolDefinition context, Scope enclosingScope)
	{
		...
		while ((index = part.IndexOf('$', index)) != -1)
		{
			...
			if (end > index)
			{
				...
				{
				case "RootNamespace":
					if (enclosingScope is CompilationUnitScope)
					{
						part = part.Insert(index, 
							#if UNITY_2020_2_OR_NEWER
							context.Assembly.scriptAssembly.rootNamespace ??
							#endif
