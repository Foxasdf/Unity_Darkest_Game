...
public static class SISettings
{
	...
	//public static BoolOption foldingBraces = Create("FoldingBraces", true);
	...
	public static BoolOption compileOnDoubleSave = Create("CompileOnDoubleSave", true);
	...
	public static void CopyPreferences()
	{
		...
		sb.Append(compileOnDoubleSave.ToJson()); sb.Append(",\n");
	...
	static void ViewSettings()
	{
		...
		labelWidth = 250f;
		//Draw(".  For Matching Braces", foldingBraces, codeFoldingCode || codeFoldingCodeInspector);
	...
	static void EditorSettings()
	{
		...
		if (!compileOnSave)
			Draw("Save Twice to Compile and Reload", compileOnDoubleSave, true);
...
Compile and reload assemblies with " + ctrlR + (compileOnDoubleSave ? " or with a 'double-save'." : "."),
