...
//*
		...
		{
			var targetTypedNewExpression = new Id("targetTypedNewExpression");
		
			parser.Add(new Rule("targetTypedNewExpression",
			...
			parser.Add(new Rule("primaryExpression",
				...
				| primaryExpressionStart - new Many(new If((new Opt("?") - (new Lit(".") | "[" | "->")) | "(" | "!", primaryExpressionPart))
				| new Seq( "new",
					new If("(", targetTypedNewExpression)
					| ((nonArrayType | ".EXPECTEDTYPE") - (objectCreationExpression | arrayCreationExpression))
					| implicitArrayCreationExpression
					| anonymousObjectCreationExpression,
					new Many(new If((new Opt("?") - (new Lit(".") | "[")) | "(" | "!" | "->", primaryExpressionPart)))
		...
		parser.Add(new Rule("objectOrCollectionInitializer",
			...
			) { semantics = SemanticFlags.FoldingScope });
	...
	public void Parse(FGTextBuffer textBuffer, string bufferName)
	{
	//	var s = new Stopwatch();
	//	s.Start();
		...
		var scanner = Scanner.New(this, textBuffer, bufferName);
		try
		{
		...
		if (node.scope == null || nodeScopeAsSDS != null && nodeScopeAsSDS.declaration == null)
		{
			...
			switch (scopeSemantics)
			{
				...
				case SemanticFlags.AttributesScope:
					...
					break;
				case SemanticFlags.FoldingScope:
					return null;
				default:
	...
	public void OnReduceSemanticNode(FGTextBuffer textBuffer, ParseTree.Node node, string fileName = null)
	{
		...
		var closeBrace = node.lastValid as ParseTree.Leaf;
		if (closeBrace != null && closeBrace.IsLit("}"))
		{
			var openBrace = node.FindLit("{");
			if (openBrace != null)
			{
				if (textBuffer != null)
				{
					var startFromPreviousToken = node.RuleName != "block" ||
						node.parent == null || node.parent.RuleName != "embeddedStatement" ||
						node.parent.parent == null || node.parent.parent.RuleName != "statement";

					var prevLeaf = startFromPreviousToken ? openBrace.FindPreviousLeaf() ?? openBrace : openBrace;
					if (prevLeaf != null)
					{
						var lineIndex = prevLeaf.token.Line;
						var from = startFromPreviousToken ? textBuffer.GetTokenSpan(prevLeaf).EndPosition : textBuffer.GetTokenSpan(openBrace).StartPosition;
						var oldFrom = startFromPreviousToken ? textBuffer.GetTokenSpan(openBrace).StartPosition : from;
						var to = textBuffer.GetTokenSpan(closeBrace).EndPosition;
						...
						if (lineIndex < to.line)
						{
							...
							if (addedSpan != span)
							{
								addedSpan.owner = node;
								addedSpan.text = " {...} ";
							}
							else if (oldFrom.line != from.line)
							{
								oldSpanIndex = textBuffer.IndexOfFirstCollapsibleTextSpanAtLine(oldFrom.line);
								if (oldSpanIndex >= 0)
								{
									oldSpan = textBuffer.collapsibleTextSpans[oldSpanIndex];
								}
								...
								if (oldSpan != null)
								{
									if (oldFrom == oldSpan.span.Start && to == oldSpan.span.End)
										textBuffer.RemoveCollapsibleTextSpan(oldSpan);
								}
							}
						}
	...
	public class Scanner : IScanner
	{
		...
		public static Scanner New(CsGrammar grammar, FGTextBuffer textBuffer, string fileName)
		{
			if (pool.Count == 0)
				return new Scanner(grammar, textBuffer, fileName);
			
			var scanner = pool.Pop();
			scanner.grammar = grammar;
			scanner.textBuffer = textBuffer;
			scanner.lines = textBuffer.formattedLines;
		...
		protected Scanner(CsGrammar grammar, FGTextBuffer textBuffer, string fileName)
		{
			this.grammar = grammar;
			this.fileName = fileName;
			this.textBuffer = textBuffer;
			lines = textBuffer.formattedLines;
		...
		{
			var clone = New(grammar, textBuffer, fileName);
			clone.tokens = tokens;
		...
		{
			grammar.OnReduceSemanticNode(textBuffer, node, fileName);
		}
